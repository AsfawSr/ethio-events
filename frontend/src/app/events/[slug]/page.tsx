...
import RideEstimateModal from '@/components/RideEstimateModal';
...
export default function EventDetailPage() {
  ...
  return (
    <div className="pb-32">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="p-6 sm:p-8 rounded-3xl bg-slate-900/90 border border-white/10 shadow-xl backdrop-blur-xl space-y-4">
              ...
              {/* Venue Info & Transportation Card */}
              <div className="p-4 rounded-2xl bg-slate-950/60 border border-white/10 space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  {/* Ride Fare Estimate & Directions Trigger */}
                  <button
                    type="button"
                    onClick={() => setShowRideModal(true)}
                    className="inline-flex items-center justify-center gap-1.5 bg-gradient-to-r from-amber-500/15 via-amber-400/15 to-yellow-500/15 hover:from-amber-500/25 hover:to-yellow-500/25 text-amber-300 hover:text-amber-200 border border-amber-500/40 px-3.5 py-2 rounded-xl text-xs font-bold transition shadow-sm active:scale-95 shrink-0"
                  >
                    <Car className="h-4 w-4 text-amber-400" />
                    <span>🚕 Estimate Taxi Fare & Directions</span>
                  </button>
                </div>
              </div>
            </div>
      ...
      {/* Ride Estimate Fare & Directions Modal */}
      {showRideModal && (
        <RideEstimateModal
          event={event}
